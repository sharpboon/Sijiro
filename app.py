...
with st.expander("💡 실시간 등급 판정 및 데이터별 이상 원인 분석 기준", expanded=False):
    ...
    with 공지_우:
        st.markdown("""
        #### 🔍 원인(`◀ 원인`) 도출 방식
        * 사용자(사이드바)가 지정한 정상 기준점에서 통계적으로 가장 멀리 이탈한 지표 색출
        * <주의> 업로드된 데이터에 빈칸(결측치)이 있을 경우, 해당 열의 전체 평균값으로 자동 보정됨
        """)
...
with 우측_화면:
    ...
    st.caption("⬇️ 통계 · TOP5 · 상세 분석은 그래프 아래에 표시")
