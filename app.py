...
st.markdown("정상 패턴(파란색)은 배경으로 깔고, **이상 패턴(빨간색)만 독보이도록 처리**했습니다. 각 축은 압축된 잠재공간의 주요 특성을 의미합니다.")
...
# 오차 정도 (위험도 점수) 추가
# decision_function은 정상일수록 양수, 이상일수록 음수를 반환하므로 보기 편하게 뒤집어서 스케일링
내부_점수 = 모델.decision_function(df[기본_특성])
df['위험도 점수'] = np.round(-내부_점수 * 100, 1) # 높을수록 비정상
...
df['잠재축 X (사용자 활동성)'] = 잠재공간[:, 0]
df['잠재축 Y (결제 및 에러 패턴)'] = 잠재공간[:, 1]
df['잠재축 Z (행동 변동성)'] = 잠재공간[:, 2]
...
with 좌측_화면:
    # 마우스 올렸을 때(Hover) 보여줄 정보와 숨길 정보 명시적 제어
    툴팁_설정 = {
        '잠재축 X (사용자 활동성)': False, # 숨김
        '잠재축 Y (결제 및 에러 패턴)': False, # 숨김
        '잠재축 Z (행동 변동성)': False, # 숨김
        '마커크기': False, # 숨김
        '상태': True,
        '위험도 점수': True,
        '체류시간': True,
    ...
    fig = px.scatter_3d(
        ...
        size='마커크기',
        size_max=12,
        hover_data=툴팁_설정, # 깔끔하게 정리된 툴팁 적용
...
with 우측_화면:
    ...
    # 표에서도 불필요한 정보 빼고 필수 특성과 위험도만 보여주기
    보여줄_컬럼 = ['상태', '위험도 점수'] + 기본_특성
    st.dataframe(
        df[df['상태'] == '이상 패턴 (차단 대상)'][보여줄_컬럼]
        .sort_values(by='위험도 점수', ascending=False) # 위험도가 가장 높은 순으로 정렬
        .head(10)
    )
