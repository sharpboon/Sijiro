...
이상_샘늘수 = st.sidebar.slider("이상 유저 데이터 수", 10, 500, 50, 10)
...
이상_데이터 = np.random.uniform(low=[10, 50, 0, 10, 10], high=[100, 200, 500, 50, 100], size=(이상_샘늘수, 5))
...
# ==========================================
좌측_화면, 우측_화면 = st.columns([3, 1.2]) # 유저 조회를 위해 우측 너비를 살짝 넓힘
...
with 우측_화면:
    ...
    if 선택된_유저 != "선택 안 함":
        ...
        with st.container(border=True):
            ...
            if 유저_DATA_상태 := 유저_데이터['상태'] != '🔵 안전 (정상 패턴)':
            ...
            지표_데이터 = {}
            for 특성 in 기본_특성:
                # HTML 태그 제거 후 깔끔한 텍스트만 추출하기 위해 백업 데이터 활용
