...
# 버튼을 누를 때마다 난수 시드를 변경하여 데이터를 완전히 새롭게 생성
if st.sidebar.button("🔄 새로운 유저 패턴으로 싹 다 바꾸기", use_container_width=True):
    st.session_state.random_seed = np.random.randint(0, 100000)

시나리오 = st.sidebar.selectbox(
    "어떤 상황을 시뮬레이션 할까요?", 
    ["1. 평범한 평일 (기본)", "2. 주말 대규모 이벤트 (활동/결제 폭주)", "3. 서버 장애 발생 (에러 폭주)"]
)
...
# ==========================================
# 데이터 생성 및 모델링 (시나리오 연동)
# ==========================================
np.random.seed(st.session_state.random_seed)

# 시나리오별 정상 유저의 기본 기준점(loc) 세팅
if "1. 평범한 평일" in 시나리오:
    정상_기준점 = [50, 10, 100, 1, 80]
elif "2. 주말 대규모 이벤트" in 시나리오:
    정상_기준점 = [90, 30, 400, 2, 150] # 체류시간, 클릭, 결제액, 스크롤 대폭 상승
elif "3. 서버 장애 발생" in 시나리오:
    정상_기준점 = [20, 5, 10, 25, 30] # 체류시간 감소, 에러수 대폭 상승
...
with st.expander("💡 실시간 등급 판정 및 데이터별 이상 원인 분석 기준 안내", expanded=True):
    ...
    with 공지_좌:
        st.markdown(f"""
        #### 📌 3단계 탐지 등급 산정 기준
        * **🔵 안전 (정상 패턴)**: 대다수의 일반 유저 행동 흐름을 따르는 안정적인 데이터입니다.
        * **🟡 주의 (관찰 요망)**: 모델 분류상 정상이지만, 위험도 점수가 높은 **상위 {주의_비율}%**의 경계선 데이터입니다.
        * **🔴 위험 (차단 대상)**: AI 모델이 명백한 이상 패턴으로 탐지한 **상위 {위험_비율}%**의 고위험군 유저입니다.
        """)
    with 공지_우:
        st.markdown("""
        #### 🔍 데이터별 동적 원인(`◀ 원인`) 도출 방식
        * 시나리오(평일/주말/에러)가 바뀔 때마다 **AI가 바뀐 정상 패턴을 실시간으로 다시 학습**합니다.
        * 각 데이터가 **'🔵 안전 등급의 평균 지표'에서 통계적으로 얼마나 멀리 벗어났는지**를 실시간 비교합니다.
        * 5개 행동 지표 중 **가장 비정상적으로 치우친 핵심 지표 하나를 추적**하여 툴팁(`◀ 원인`)에 색상으로 강조합니다.
        """)
