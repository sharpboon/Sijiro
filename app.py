...
# ==========================================
# [핵심 변경] 데이터 생성 로직 (슬라이더로부터 독립)
# ==========================================
np.random.seed(st.session_state.random_seed)

# 유저 데이터 자체는 시나리오 고유의 기본 표준값으로만 생성 (슬라이더를 움직여도 데이터 원본은 고정!)
시나리오_원래값 = SCENARIO_DEFAULTS[시나리오]
진짜_기본_기준점 = [시나리오_원래값['체류시간'], 시나리오_원래값['클릭수'], 시나리오_원래값['결제액'], 시나리오_원래값['에러수'], 시나리오_원래값['스크롤깊이']]

정상_데이터 = np.random.normal(loc=진짜_기본_기준점, scale=[10, 2, 20, 0.5, 10], size=(정상_샘플수, 5))
...
# ==========================================
# [핵심 변경] 관리자 설정 기준 실시간 위험도 및 편차 계산
# ==========================================
설정_기준점 = np.array([정상_체류, 정상_클릭, 정상_결제, 정상_에러, 정상_스크롤])
특성_표준편차 = df[기본_특성].std().replace(0, 1).values

# 내가 바꾼 슬라이더 기준점으로부터 유저가 얼마나 멀리 떨어져 있는지 실시간 거리 계산
거리_점수 = np.sqrt(np.sum(((df[기본_특성].values - 설정_기준점) / 특성_표준편차) ** 2, axis=1))
df['위험도 점수'] = np.round(거리_점수 * 15, 1)

# 민감도 설정에 따른 동적 등급 컷오프
...
표준점수 = np.abs((df[기본_특성] - 설정_시리즈) / df[기본_특성].std().replace(0, 1))
df['주요원인_특성'] = 표준점수.idxmax(axis=1)

def format_hover_text(row, col, 기준값):
    값 = row[col]
    편차 = 값 - 기준값  # 고정된 값에서 내가 실시간으로 조절한 기준값을 뺌!
...
# 각 특성별로 슬라이더 기준값을 대입하여 텍스트 포맷팅 생성
...
with st.expander("💡 실시간 등급 판정 및 데이터별 이상 원인 분석 기준 안내", expanded=True):
    ...
    with 공지_좌:
        st.markdown(f"""
        #### 📌 3단계 탐지 등급 산정 기준
        * **🔵 안전 (정상 패턴)**: 관리자가 슬라이더로 설정한 기준점 근처에 위치한 유저들입니다.
        * **🟡 주의 (관찰 요망)**: 수동 설정된 기준에서 멀어져 위험도 점수가 높은 **상위 {주의_비율}%**의 경계 유저입니다.
        * **🔴 위험 (차단 대상)**: 현재 설정된 기준점과 데이터 괴리가 매우 심각한 **상위 {위험_비율}%**의 고위험 유저입니다.
        """)
    with 공지_우:
        st.markdown("""
        #### 🔍 데이터별 동적 원인(`◀ 원인`) 도출 방식
        * 유저들의 원래 고유 수치는 고정된 상태에서, 사이드바의 **정상치 기준값 수정을 기준으로 편차를 실시간 재연산**합니다.
        * 유저의 원래 값이 내가 설정한 기준 평균치에서 가장 멀리 이탈한 지표에 `◀ 원인` 표시가 붙습니다.
        """)
