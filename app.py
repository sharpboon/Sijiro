...
# [핵심] 사용자가 시나리오를 변경하면 해당 시나리오의 기본값으로 슬라이더 수치 강제 업데이트
...
# ==========================================
# 데이터 생성 및 모델링 (수동 설정값 연동)
# ==========================================
np.random.seed(st.session_state.random_seed)

# 사용자가 조절한 슬라이더 값을 정상 데이터의 중심점(loc)으로 사용
정상_기준점 = [정상_체류, 정상_클릭, 정상_결제, 정상_에러, 정상_스크롤]
정상_데이터 = np.random.normal(loc=정상_기준점, scale=[10, 2, 20, 0.5, 10], size=(정상_샘플수, 5))

# 이상 데이터는 고정된 넓은 범위에서 무작위 추출하여 정상 기준점이 바뀔 때 탐지 양상을 관찰할 수 있게 함
