...
# 슬라이더 대신 숫자 입력창으로 변경하여 대량의 데이터도 쉽게 입력 가능
정상_샘플수 = st.sidebar.number_input("일반 유저 데이터 수", min_value=10, max_value=100000, value=800, step=100)
이상_샘플수 = st.sidebar.number_input("이상 유저 데이터 수", min_value=1, max_value=50000, value=50, step=10)

st.sidebar.markdown("---")
st.sidebar.markdown("### 🚨 판정 민감도 (점수 기준)")
# % 비율 대신 절대적인 '점수' 기준으로 변경
위험_점수 = st.sidebar.number_input("🔴 위험(차단) 기준 점수", min_value=1, max_value=100, value=80, step=1)
주의_점수 = st.sidebar.number_input("🟡 주의(관찰) 기준 점수", min_value=1, max_value=100, value=50, step=1)

if 주의_점수 >= 위험_점수:
    st.sidebar.error("⚠️ 오류: '위험' 기준 점수가 '주의' 기준 점수보다 높아야 합니다.")
...
# 사용자가 입력한 점수를 바탕으로 조건 설정
조건_위험 = df['위험도 점수'] >= 위험_점수
조건_주의 = df['위험도 점수'] >= 주의_점수

# np.select는 순차적으로 조건을 검사하므로 위험 -> 주의 순으로 넣으면 됩니다.
...
with st.expander("💡 실시간 등급 판정 및 데이터별 이상 원인 분석 기준 안내", expanded=False):
    ...
    with 공지_좌:
        ...
        #### 📌 3단계 탐지 등급 산정 기준
        * **🔵 안전**: 위험 점수 **{주의_점수}점 미만**
        * **🟡 주의**: 위험 점수 **{주의_점수}점 이상 ~ {위험_점수}점 미만**
        * **🔴 위험**: 위험 점수 **{위험_점수}점 이상**
        """)
    with 공지_우:
        st.markdown("""
